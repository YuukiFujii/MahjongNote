...
open class LineChart: UIView {
    ...
    open var colors: [UIColor] = [
        ...
        UIColor(red: 0.89, green: 0.29, blue: 0.29, alpha: 1),
    ...
    fileprivate func highlightDataPoints(_ index: Int) {
        for (lineIndex, dotsData) in dotsDataStore.enumerated() {
            ...
            if (dataStore[0][index] >= 0) {
                dot.backgroundColor = Helpers.lightenUIColor(colors[0]).cgColor
            } else {
                dot.backgroundColor = Helpers.lightenUIColor(colors[1]).cgColor
            }
